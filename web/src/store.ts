...
import { fetchLatest, fetchMetrics, fetchStatus, fetchErrors } from "./api/endpoints";
  ...
  loadAll: async () => {
    ...
    try {
      const [latest, metrics, status, errors] = await Promise.all([
        fetchLatest(), fetchMetrics(), fetchStatus(), fetchErrors()
      ]);
      set({ latest, metrics, status, errors, loading: false });

      // 緩存最後一次可用資料（API 掛掉時顯示）
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ latest, metrics, status }));
    } catch (e: any) {
  ...
  pollLatest: (ms = 3000) => {
    ...
    const tick = async () => {
      try {
        ...
        const metrics = [...get().metrics, { ts: latest.ts, temp: latest.temp, hum: latest.hum }]
          .slice(-300); // 保持最多 300 點
        set({ latest, metrics, lastError: undefined });
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ latest, metrics, status: get().status }));
      } catch (e: any) {
